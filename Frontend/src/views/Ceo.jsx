...
export default function Ceo() {
    ...
    const [savingIntermediario, setSavingIntermediario] = useState(false);
    const [editingIntermediarioId, setEditingIntermediarioId] = useState(null);
    const [editingIntermediarioData, setEditingIntermediarioData] = useState({ nombre: '', porcentaje: '' });
    const [mesIntermediarios, setMesIntermediarios] = useState(() => {
        const d = new Date();
        return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    });
    const [intermediariosResumen, setIntermediariosResumen] = useState({ totalComisiones: 0, data: [] });
    const [loadingIntermediariosResumen, setLoadingIntermediariosResumen] = useState(false);
    const [detalleIntermediarioModal, setDetalleIntermediarioModal] = useState({ open: false, item: null });
    ...
    const [editViajeModal, setEditViajeModal] = useState({ open: false, id: null, data: { origen: '', destino: '', fecha: '', camionId: '', acopladoId: '', tipoMercaderia: '', cliente: '', intermediarioId: '' }, loading: false, error: '' });
    const openEditViaje = (v) => {
        setEditViajeModal({
            ...
            data: {
                ...
                cliente: v.cliente || '',
                intermediarioId: v.intermediario?.id || v.intermediarioId || ''
            },
            loading: false,
            error: ''
        });
    };
    const closeEditViaje = () => setEditViajeModal({ open: false, id: null, data: { origen: '', destino: '', fecha: '', camionId: '', acopladoId: '', tipoMercaderia: '', cliente: '', intermediarioId: '' }, loading: false, error: '' });
    const saveEditViaje = async () => {
        ...
        try {
            ...
            const body = {
                ...
                intermediarioId: editViajeModal.data.intermediarioId ? Number(editViajeModal.data.intermediarioId) : null,
            ...
            const intermediarioSeleccionado = intermediarios.find(c => c.id === body.intermediarioId) || null;
            ...
            setViajes(prev => (prev || []).map(v => {
                ...
                return {
                    ...
                    intermediarioId: body.intermediarioId,
                    intermediario: intermediarioSeleccionado || null
                };
            }));
    ...
    const fetchIntermediarios = async () => {
        try {
            ...
            const list = Array.isArray(data) ? data : (data.data || []);
            setIntermediarios(list);
            return list;
        } catch (e) {
            console.error('Error cargando intermediarios:', e);
            setIntermediarios([]);
            return [];
        }
    };

    const fetchIntermediariosResumen = async (mesValue = mesIntermediarios) => {
        if (!mesValue) return { totalComisiones: 0, data: [] };
        const [anio, mes] = String(mesValue).split('-').map(Number);
        try {
            setLoadingIntermediariosResumen(true);
            const { data } = await api.get(`/intermediarios/resumen?mes=${mes}&anio=${anio}`);
            const normalized = {
                totalComisiones: Number(data?.totalComisiones || 0),
                data: Array.isArray(data?.data) ? data.data : []
            };
            setIntermediariosResumen(normalized);
            return normalized;
        } catch (e) {
            showToast(e?.response?.data?.error || 'Error cargando resumen de intermediarios', 'error');
            setIntermediariosResumen({ totalComisiones: 0, data: [] });
            return { totalComisiones: 0, data: [] };
        } finally {
            setLoadingIntermediariosResumen(false);
        }
    };

    const crearIntermediario = async (e) => {
        e.preventDefault();
        setSavingIntermediario(true);
        try {
            const body = {
                nombre: (nuevoIntermediario.nombre || '').trim(),
                porcentaje: Number(nuevoIntermediario.porcentaje || 0)
            };
            if (!body.nombre) {
                showToast('El nombre del intermediario es requerido', 'error');
                setSavingIntermediario(false);
                return;
            }
            await api.post('/intermediarios', body);
            setNuevoIntermediario({ nombre: '', porcentaje: '' });
            await Promise.all([fetchIntermediarios(), fetchIntermediariosResumen()]);
            showToast('Intermediario creado', 'success');
        } catch (e) {
            const msg = e?.response?.data?.error || 'Error creando intermediario';
            setError(msg);
            showToast(msg, 'error');
        } finally {
            setSavingIntermediario(false);
    ...
    const guardarIntermediario = async (id) => {
        try {
            const body = {
                nombre: (editingIntermediarioData.nombre || '').trim(),
                porcentaje: Number(editingIntermediarioData.porcentaje || 0)
            };
            if (!body.nombre) {
                showToast('El nombre del intermediario es requerido', 'error');
                return;
            }
            await api.put(`/intermediarios/${id}`, body);
            setEditingIntermediarioId(null);
            await Promise.all([fetchIntermediarios(), fetchIntermediariosResumen()]);
            showToast('Intermediario actualizado', 'success');
        } catch (e) {
            showToast(e?.response?.data?.error || 'Error actualizando intermediario', 'error');
        }
    };

    const eliminarIntermediario = async (id, nombre) => {
        if (!window.confirm(`¿Eliminar intermediario "${nombre}"?`)) return;
        try {
            await api.delete(`/intermediarios/${id}`);
            if (editingIntermediarioId === id) setEditingIntermediarioId(null);
            await Promise.all([fetchIntermediarios(), fetchIntermediariosResumen()]);
            showToast('Intermediario eliminado', 'success');
        } catch (e) {
            showToast(e?.response?.data?.error || 'Error eliminando intermediario', 'error');
        }
    };
    ...
    const finalizarViaje = async (id) => {
        ...
        try {
            ...
            const resumenActualizado = await fetchIntermediariosResumen();
            if (detalleIntermediarioModal.open && detalleIntermediarioModal.item?.intermediarioId) {
                const updatedItem = (resumenActualizado?.data || []).find(
                    x => x.intermediarioId === detalleIntermediarioModal.item.intermediarioId
                );
                if (updatedItem) {
                    setDetalleIntermediarioModal({ open: true, item: updatedItem });
                }
            }
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            setError('');
            try { await Promise.all([fetchCamiones(), fetchAcoplados(), fetchUsuarios(), fetchClientes(), fetchIntermediarios(), fetchStats(), fetchIntermediariosResumen()]); }
    ...
    useEffect(() => {
        fetchIntermediariosResumen(mesIntermediarios);
    }, [mesIntermediarios]);
    ...
    const openPeriodoComisionesModal = () => {
        setPeriodoComisionesModal({ open: true, value: mesIntermediarios });
    };
    ...
    const applyPeriodoComisionesModal = () => {
        if (periodoComisionesModal.value) {
            setMesIntermediarios(periodoComisionesModal.value);
        }
    ...
    // Modal para gestionar adelantos existentes (historial por mes/año)
    ...
    const saveEditAdelanto = async (id) => {
        try {
            ...
            cancelEditAdelanto();
            // Recargar manteniendo mes/año seleccionados
    ...
    return (
        <>
            ...
            <div className="container py-3 space-y-4">
                    ...
                    <div className="d-flex align-items-center gap-2">
                        ...
                        {user?.rol === 'ceo' && (
                            <>
                                ...
                                {/* Botón de Historial de Adelantos */}
                                <button className="btn btn-outline-success" onClick={openGestionAdelantos} title="Ver historial de adelantos por mes">
                        ...
                        <div className="card shadow-sm">
                            <div className="card-body">
                                ...
                                <form onSubmit={crearViaje} className="row g-2 mt-2" style={{ opacity: savingViaje ? 0.85 : 1 }}>
                                    ...
                                    <div className="col-6">
                                        <select className="form-select" value={nuevoViaje.intermediarioId || ''} onChange={e => setNuevoViaje(v => ({ ...v, intermediarioId: e.target.value }))}>
                                            <option value="">Sin intermediario</option>
                                            {intermediarios.map(c => <option key={c.id} value={c.id}>{c.nombre} ({Number(c.porcentaje || 0)}%)</option>)}
                                        </select>
                                    </div>
                                    <div className="col-6">
                        ...
                        <div className="table-responsive">
                            {viajesFiltrados.length === 0 ? (
                                ...
                                <table className={`table table-sm table-striped table-hover table-sticky table-cols-bordered`}>
                                    <thead>
                                        <tr>
                                            {['Fecha', 'Estado', 'Origen', 'Destino', 'Camión', 'Acoplado', 'Camionero', 'Tipo', 'Cliente', 'Intermediario', 'Km', 'Toneladas', 'Precio/Tn', 'Importe'].map((label) => (
                                                <th
                                    ...
                                    <tbody>
                                        {viajesPagina.map(v => (
                                            <tr key={v.id} className={getFacturaRowClass(v.facturaEstado)}>
                                                ...
                                                <td title={v.intermediario?.nombre || '-'}>{v.intermediario?.nombre || '-'}</td>
                ...
                <div className="row g-3 mt-3">
                    <div className="col-12 col-xl-5">
                        <div className="card shadow-sm h-100">
                            <div className="card-body">
                                <h3 className="h5">Intermediarios</h3>
                                <form onSubmit={crearIntermediario} className="row g-2 mt-2" style={{ opacity: savingIntermediario ? 0.85 : 1 }}>
                                    <div className="col-8">
                                        <input
                                            className="form-control"
                                            placeholder="Nombre del intermediario"
                                            value={nuevoIntermediario.nombre}
                                            onChange={e => setNuevoIntermediario(v => ({ ...v, nombre: e.target.value }))}
                                        />
                                    </div>
                                    <div className="col-4">
                                        <input
                                            ...
                                            step={0.01}
                                            placeholder="%"
                                            value={nuevoIntermediario.porcentaje}
                                            onChange={e => setNuevoIntermediario(v => ({ ...v, porcentaje: e.target.value }))}
                                        />
                                    </div>
                                    <div className="col-12">
                                        <button className="btn btn-primary" disabled={savingIntermediario}>{savingIntermediario ? 'Creando…' : 'Crear intermediario'}</button>
                                    </div>
                                </form>

                                <div className="table-responsive mt-3">
                                    {intermediarios.length === 0 ? (
                                        <EmptyState title="Sin intermediarios" description="Todavía no cargaste intermediarios" />
                                    ) : (
                                        <table className="table table-sm table-hover align-middle mb-0 table-sticky table-cols-bordered">
                                            <thead>
                                                <tr>
                                            ...
                                            <tbody>
                                                {intermediarios.map(c => (
                                                    <tr key={c.id}>
                                                        <td>
                                                            {editingIntermediarioId === c.id ? (
                                                                <input className="form-control form-control-sm" value={editingIntermediarioData.nombre} onChange={e => setEditingIntermediarioData(v => ({ ...v, nombre: e.target.value }))} />
                                                            ) : c.nombre}
                                                        </td>
                                                        <td style={{ maxWidth: 120 }}>
                                                            {editingIntermediarioId === c.id ? (
                                                                <input className="form-control form-control-sm" type="number" min={0} max={100} step={0.01} value={editingIntermediarioData.porcentaje} onChange={e => setEditingIntermediarioData(v => ({ ...v, porcentaje: e.target.value }))} />
                                                            ) : `${Number(c.porcentaje || 0).toFixed(2)}%`}
                                                        </td>
                                                        <td className="text-end">
                                                            {editingIntermediarioId === c.id ? (
                                                                <div className="btn-group btn-group-sm">
                                                                    <button className="btn btn-success" onClick={() => guardarIntermediario(c.id)}><i className="bi bi-check-lg"></i></button>
                                                                    <button className="btn btn-outline-secondary" onClick={() => setEditingIntermediarioId(null)}><i className="bi bi-x-lg"></i></button>
                                                                </div>
                                                            ) : (
                                                                <div className="btn-group btn-group-sm">
                                                                    <button className="btn btn-sm btn-outline-primary" onClick={() => { setEditingIntermediarioId(c.id); setEditingIntermediarioData({ nombre: c.nombre, porcentaje: String(Number(c.porcentaje || 0)) }); }} title="Editar">
                                                                        <i className="bi bi-pencil"></i>
                                                                    </button>
                                                                    <button className="btn btn-sm btn-outline-danger" onClick={() => eliminarIntermediario(c.id, c.nombre)} title="Eliminar">
                                                                        <i className="bi bi-trash"></i>
                                                                    </button>
                                                                </div>
                                                            )}
                            ...
                            <div className="card-body">
                                <div className="d-flex flex-wrap align-items-end gap-3 mb-3">
                                    <div className="me-auto">
                                        <h3 className="h5 mb-1">Comisiones por intermediario</h3>
                                    ...
                                    >
                                        ...
                                        <span className="fw-semibold">{formatMonthYear(mesIntermediarios)}</span>
                                    </button>
                                </div>
                                ...
                                <div className="p-3 mb-3 rounded border border-success-subtle bg-success bg-opacity-10">
                                    ...
                                    <div className="fs-4 fw-bold text-success">$ {Number(intermediariosResumen.totalComisiones || 0).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                                </div>
                                            ...
                                            {loadingIntermediariosResumen ? (
                                                ...
                                                intermediariosResumen.data.map(item => (
                                                    <tr key={item.intermediarioId}>
                                                        ...
                                                        <td className="text-end">
                                                            <button className="btn btn-sm btn-outline-primary" onClick={() => setDetalleIntermediarioModal({ open: true, item })}>Ver viajes</button>
                                                        </td>
                                                    </tr>
                                                ))
                                            )}
                ...
                {/* Modal detalle intermediario */}
                <div className={`modal ${detalleIntermediarioModal.open ? 'show d-block' : 'fade'}`} tabIndex="-1" aria-hidden={!detalleIntermediarioModal.open}>
                    <div className="modal-dialog modal-xl modal-dialog-scrollable">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h1 className="modal-title fs-5">
                                    Detalle de comisiones - {detalleIntermediarioModal.item?.nombre || 'Intermediario'}
                                </h1>
                                <button type="button" className="btn-close" onClick={() => setDetalleIntermediarioModal({ open: false, item: null })}></button>
                            </div>
                            <div className="modal-body">
                                {!detalleIntermediarioModal.item ? (
                                    <div className="text-body-secondary">Sin datos para mostrar.</div>
                                ) : (
                                    <>
                                        <div className="d-flex flex-wrap gap-3 mb-3">
                                            <div className="small text-body-secondary">Viajes: <strong>{detalleIntermediarioModal.item.totalViajes}</strong></div>
                                            <div className="small text-body-secondary">Total importes: <strong>$ {Number(detalleIntermediarioModal.item.totalImporte || 0).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></div>
                                            <div className="small text-success-emphasis">Total comisiones: <strong>$ {Number(detalleIntermediarioModal.item.totalComision || 0).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong></div>
                                        </div>
                                        <div className="table-responsive">
                                            <table className="table table-sm table-hover align-middle">
                                                ...
                                                <tbody>
                                                    {(detalleIntermediarioModal.item.viajes || []).length === 0 ? (
                                                        <tr><td colSpan={7} className="text-center text-body-secondary py-3">No hay viajes para este intermediario en el mes seleccionado.</td></tr>
                                                    ) : (
                                                        (detalleIntermediarioModal.item.viajes || []).map(v => (
                                                            <tr key={v.id}>
                                                                <td>{formatDateOnly(v.fecha)}</td>
                                                                <td>{v.origen || '-'}</td>
                                                                <td>{v.destino || '-'}</td>
                ...
                {detalleIntermediarioModal.open && <div className="modal-backdrop show"></div>}
                        ...
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                <div className="row g-2">
                                    ...
                                    <div className="col-6"><label className="form-label">Intermediario</label>
                                        <select className="form-select" value={editViajeModal.data.intermediarioId || ''} onChange={e => setEditViajeModal(m => ({ ...m, data: { ...m.data, intermediarioId: e.target.value } }))}>
                                            <option value="">Sin intermediario</option>
                                            {intermediarios.map(c => <option key={c.id} value={c.id}>{c.nombre} ({Number(c.porcentaje || 0)}%)</option>)}
                                        </select>
                                    </div>
